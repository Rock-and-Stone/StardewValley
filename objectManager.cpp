...
void objectManager::SetWood(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
         ...
         if (amount < 10)
         {
             Wood->init(OBJ_WOOD, "springObjects", 6, 12, PointMake(x + (5 * i), y + (5 * i)),2);
         }
         else
             Wood->init(OBJ_WOOD, "springObjects", 6, 12, PointMake(x, y) , 2);
...
void objectManager::SetStone(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object*  Stone;
        Stone = new stone;
        Stone->init(OBJ_STONE, "springObjects", 6, 12, PointMake(x + (i * 10), y) , 1);
...
void objectManager::SetPufferFish(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        ...
        PufferFish->init(OBJ_PUFFERFISH, "springObjects", 6, 12, PointMake(x + (i * 10), y), 12);
...
void objectManager::SetTuna(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Tuna;
        Tuna = new tuna;
        Tuna->init(OBJ_TUNA, "springObjects", 6, 12, PointMake(x + (i * 10), y), 13);
...
void objectManager::SetHaliBut(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Halibut;
        Halibut = new haliBut;
        Halibut->init(OBJ_HALIBUT, "springObjects", 6, 12, PointMake(x + (i * 10), y),11);
    ...
    for (int i = 0; i < amount; i++)
{
    ...
    CauliSeed->init(OBJ_CAULIFLOWERSEED,"2",2,2,PointMake(x,y) , 17);
...
void objectManager::SetParsnipSeed(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* ParsnipSeed;
        ParsnipSeed = new axe;
        ParsnipSeed->init(OBJ_PARSNIPSEED,"2",2,2,PointMake(x,y), 19);
...
void objectManager::SetPotatoSeed(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        ...
        PotatoSeed->init(OBJ_POTATOSEED, "2", 2, 2, PointMake(x, y), 20);
...
void objectManager::SetKaleSeed(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        ...
        KaleSeed->init(OBJ_KALESEED, "2", 2, 2, PointMake(x, y), 18);
...
void objectManager::SetCauliFlower(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        ...
        CauliFlower->init(OBJ_PARSNIPSEED, "2", 2, 2, PointMake(x, y), 17);
...
void objectManager::SetParsnip(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Parsnip;
        Parsnip = new parsnip;
        Parsnip->init(OBJ_PARSNIP, "2", 2, 2, PointMake(x, y), 9);
...
void objectManager::SetPotato(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Potato;
        Potato = new potato;
        Potato->init(OBJ_POTATO, "2", 2, 2, PointMake(x, y) , 10);
...
void objectManager::SetKale(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Kale;
        Kale = new kale;
        Kale->init(OBJ_KALE, "2", 2, 2, PointMake(x, y) , 8);
...
void objectManager::SetCopper(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Copper;
        Copper = new copper;
        Copper->init(OBJ_COPPER, "2", 2, 2, PointMake(x, y), 14);
...
void objectManager::SetIron(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Iron;
        Iron = new iron;
        Iron->init(OBJ_PARSNIPSEED, "2", 2, 2, PointMake(x, y) , 15);
...
void objectManager::SetGold(float x, float y, int amount)
{
    for (int i = 0; i < amount; i++)
    {
        object* Gold;
        Gold = new gold;
        Gold->init(OBJ_GOLD, "2", 2, 2, PointMake(x, y),16);
